...
function validaCNPJ(cnpj) {
  ...
  function enviarDados() {
    ...
    // Faça uma solicitação POST para a API ASP.NET Core
    fetch("https://localhost:7007/api/inserir-cliente", {
        method: "POST",
        headers: {
...
function pesquisar(){
  ...
  fetch("https://localhost:7007/api/consulta-cliente?cpf_cnpj="+pesquisar.value, {
    method: "GET",
        headers: {
            "Content-Type": "application/json"
        },
  ...
  .then(response => response.text())
    .then(result => {
        ...
        if (result != "CPF ou CNPJ não encontrado"){
          jsonResponse = JSON.parse(result)
          divResultado.style.display = "inline";
          rCpf_cnpj.value = jsonResponse.cpf_cnpj;
          rNomeRazao.value = jsonResponse.nome_razao_social;
          rNomeFantasia.value = jsonResponse.nome_fantasia;
  
          if (rNomeFantasia.value != "" & rNomeFantasia.value != null){
              divfantasia.style.display = "inline";
          }
  
          rTelefone.value = jsonResponse.telefone;
          rCelular.value = jsonResponse.celular;
          rEmail.value = jsonResponse.email;
          rCep.value = jsonResponse.cep;
          rLogradouro.value = jsonResponse.logradouro;
          rComplemento.value = jsonResponse.complemento;
          rNumero.value = jsonResponse.numero;
          rBairro.value = jsonResponse.bairro;
          rCidade.value = jsonResponse.cidade;
          rEstado.value = jsonResponse.estado;
          rPais.value = jsonResponse.pais;
        }
        else{
          alert(result);
          divResultado.style.display = "none";
...
fetch("https://localhost:7007/api/atualiza-cliente", {
  method: "POST",
      headers: {
          "Content-Type": "application/json"
      },
...
function deletaCliente() {
  ...
  if(confirm("Confirma a exclusão?")){
    ...
    fetch("https://localhost:7007/api/delete-cliente?cpf_cnpj="+cpf_cnpj,{
      method: "DELETE",
      headers: {
        "Content-Type": "application/json"
      }
